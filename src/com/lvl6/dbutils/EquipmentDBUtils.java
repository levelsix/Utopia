...
public class EquipmentDBUtils {
  ...
  private static Equipment convertRSRowToEquipment(ResultSet rs) throws SQLException {
    ...
    if (coinPrice > 0) {
      equip = new Equipment(id, name, type, attackBoost, defenseBoost, minLevel, coinPrice, Equipment.NOT_SET);
    } else {  //this should mean diamondPrice > 0.
      equip = new Equipment(id, name, type, attackBoost, defenseBoost, minLevel, Equipment.NOT_SET, diamondPrice);      
    }
    return equip;
  }
}
