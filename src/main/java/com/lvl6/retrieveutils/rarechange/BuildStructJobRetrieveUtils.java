...
public class BuildStructJobRetrieveUtils {
	...
	public static Map<Integer, BuildStructJob> getBuildStructJobsForBuildStructJobIds(
			List<Integer> ids) {
	...
	public static BuildStructJob getBuildStructJobForBuildStructJobId(
			int buildStructJobId) {
		log.debug("retrieving build struct job data for build struct job id "
				+ buildStructJobId);
		if (buildStructJobIdsToBuildStructJobs == null) {
			setStaticBuildStructJobIdsToBuildStructJobs();
		}
		...
		if (rs != null) {
			try {
				...
				while (rs.next()) { // should only be one
					...
					if (bsj != null)
						buildStructJobIdsToBuildStructJobsTemp.put(bsj.getId(),
								bsj);
				}
	 ...
	 */
	private static BuildStructJob convertRSRowToBuildStructJob(ResultSet rs)
			throws SQLException {
		int i = 1;
		int id = rs.getInt(i++);
