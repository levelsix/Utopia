...
public class APNSWriter extends Wrap {
	...
	private void handlePostOnPlayerWallNotification(ApnsService service,
		...
		// PayloadBuilder pb =
		// APNS.newPayload().actionKey("View").badge(user.getNumBadges()+1);
		...
		if (poster != null) {
			pb.alertBody(poster.getName()
					+ " just posted on your wall! Check out the message.");

			if (!pb.isTooLong()) {
				log.info("Pushing apns message");
				service.push(token, pb.build());
				// if (user.updateRelativeBadge(1)) {
				// log.error("problem with pushing notification to user " +
				// user);
				// }
			}else {
	...
	// private void handleReferralCodeUsedNotification(ApnsService service,
	// ReferralCodeUsedResponseEvent event, User user, String token) {
	// PayloadBuilder pb =
	// APNS.newPayload().actionKey("Use").badge(user.getNumBadges()+1);
	//
	// ReferralCodeUsedResponseProto resProto =
	// event.getReferralCodeUsedResponseProto();
	// pb.alertBody("Congrats! You received free silver because " +
	// resProto.getReferredPlayer().getName() + " used your referral code.");
	//
	// if (!pb.isTooLong()) {
	// service.push(token, pb.build());
	// if (user.updateRelativeBadge(1)) {
	...
	private void handlePurchaseFromMarketplaceNotification(ApnsService service,
		...
		// PayloadBuilder pb =
		// APNS.newPayload().actionKey("Redeem").badge(user.getNumBadges()+1).alertBody("Someone purchased your equipment in the marketplace. Redeem your earnings!");
		PayloadBuilder pb = APNS
				.newPayload()
				.actionKey("Redeem")
				.badge(1)
				.alertBody(
						"Someone purchased your equipment in the marketplace. Redeem your earnings!");
	...
	private void handleBattleNotification(ApnsService service,
		...
		if ((user.getNumBadges() < SOFT_MAX_NOTIFICATION_BADGES && (lastBattleNotificationTime == null || now
				...
				|| (lastBattleNotificationTime != null && lastBattleNotificationTime
						.getTime() + 60000 * MINUTES_BEFORE_IGNORE_BADGE_CAP < now
						.getTime())) {
			...
			if (!pb.isTooLong()) {
				...
				if (!user
						.updateRelativeBadgeAbsoluteLastbattlenotificationtime(
								0, new Timestamp(now.getTime()))) {
					log.error("problem with pushing notification to user "
							+ user);
				}
			}else {
