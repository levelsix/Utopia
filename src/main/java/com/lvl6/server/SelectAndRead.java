...
public class SelectAndRead extends Thread {
	...
	public void addNewClient(SocketChannel clientChannel) {
		...
		// force selector to return so our new client can get in the loop right
		// away
		selector.wakeup();
	}
	...
	public void run() {
		try {
			...
			log.error("exception while opening Selector", e);
		}
	}
	...
	private void removePlayerFromServer(SocketChannel channel) {
		...
		// }
		// } else {
		// log.error("cannot update last logout because there is no player on channel "
		// + channel);
		// }
	}
	...
	private void delegateEvent(RequestEvent event, SocketChannel channel, EventProtocolRequest eventType) {
		...
		if (p != null) {
			...
			// log.warn("player " + p.getPlayerId() +
			// " is on a new channel, must be reconnected.");
			...
			// // Save the socket into another array so that controller can
			// retrieve later
