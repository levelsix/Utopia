...
//
//      boolean legitCreate = checkLegiteate(resBuilder, user, clanName, tag);
//
...
//      resBuilder.setStatus(RequestJoinClanStatus.NOT_ENOUGH_DIAMONDS);
...
//      resBuilder.setStatus(RequestJoinClanStatus.INVALID_TAG_LENGTH);
...
//        resBuilder.setStatus(RequestJoinClanStatus.NAME_TAKEN);
//        log.error("clan name already taken with name " + clanName);
//        return false;
//      }
//      if (clan.getTag().equalsIgnoreCase(tag)) {
//        resBuilder.setStatus(RequestJoinClanStatus.TAG_TAKEN);
