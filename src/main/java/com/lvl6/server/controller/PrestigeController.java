...
import com.lvl6.utils.utilmethods.InsertUtils;
  ...
  private boolean writeChangesToDB(Builder resBuilder, User user) {
    ...
    if (0 >= numDeleted) {
      log.error("unexpected error: no user cities were deleted for user. numDeleted="
          + numDeleted + ", user=" + user);
    }
    //user_quests delete all of them
    numDeleted = DeleteUtils.get().deleteAllUserQuestsForUser(userId);
    if (0 >= numDeleted) {
      log.error("unexpected error: no user quests were deleted for user. numDeleted="
          + numDeleted + ", user=" + user);
    }
    ...
    if (0 >= numDeleted) {
      log.error("unexpected error: no user quests completed defeat type jobs were deleted" +
      		" for user. numDeleted=" + numDeleted + ", user=" + user);
    }
    ...
    if (0 >= numDeleted) {
      log.error("unexpected error: no user quests completed tasks were deleted for user." +
      		" numDeleted=" + numDeleted + ", user=" + user);
    }
    ...
    if (0 >= numDeleted) {
      log.error("unexpected error: no user quests defeat type job progress were deleted for" +
      		" user. numDeleted=" + numDeleted + ", user=" + user);
    }
    //user_quests_task_progress
    numDeleted = DeleteUtils.get().deleteAllUserQuestsTaskProgress(userId);
    if (0 >= numDeleted) {
      log.error("unexpected error: no user quests task progress were deleted for user. numDeleted="
          + numDeleted + ", user=" + user);
    }
    //user_tasks
    numDeleted = DeleteUtils.get().deleteAllUserTasksForUser(userId);
    if (0 >= numDeleted) {
      log.error("unexpected error: no user tasks were deleted for user. numDeleted="
          + numDeleted + ", user=" + user);
    }
